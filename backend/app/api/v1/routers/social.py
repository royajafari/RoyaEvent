...
from app.schemas.social import MyFollowersOut, MyFollowsDetailOut
...
@router.get("/me/followers", response_model=MyFollowersOut)
def list_my_followers(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    return social_service.list_my_followers(db, current_user.id)
