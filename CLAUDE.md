...
RoyaEvent/
  ...
  frontend/src/
    app/
      error.tsx                    # ErrorBoundary سراسری (segment error boundary خودکار نکست‌جس، فاز ۱۰) — خطای شبکه رو از خطای رندر جدا می‌کنه
      global-error.tsx             # ErrorBoundary برای کرش خود root layout (فاز ۱۰) — خودش <html>/<body> کامل داره
    ...
    components/NetworkStatusGate.tsx # فاز ۱۰ — مقاومت آفلاین (نیازمندی ۳۳ architecture.md)، دور SiteHeader/children/SiteFooter در layout ریشه (client)
    ...
    hooks/useNetworkStatus.ts  # فاز ۱۰ — رویداد online/offline مرورگر + پینگ دوره‌ای /health، مصرف‌شده در NetworkStatusGate
    fonts/kalameh.ts
...
24. **خوندن یه API فقط-کلاینتی (مثل `navigator.onLine`، `window.matchMedia`، `localStorage`) مستقیم داخل خود `useState` initializer، یه hydration mismatch واقعی می‌سازه، نه فقط تئوری.** سرور همچین APIای رو اصلاً نداره، پس همیشه یه مقدار پیش‌فرض ثابت رندر می‌کنه؛ اگه initializer کلاینت مقدار واقعی رو بلافاصله (پیش از هر effect) بخونه و اون مقدار با پیش‌فرض سرور فرق کنه، اولین رندر کلاینت با HTML سرور مطابقت نداره و React خطای hydration می‌ده (`useNetworkStatus`، فاز ۱۰ — کشف‌شده از روی لاگ واقعی dev سرور، نه حدس). راه‌حل درست: مقدار اولیه‌ی `useState` همیشه همون پیش‌فرض SSR-امن (مثلاً `true`)، مقدار واقعی فقط داخل `useEffect` (که تضمینی فقط بعد از hydrate اجرا می‌شه) تصحیح بشه.
25. **pool پیش‌فرض Vitest (`forks`، بر پایه‌ی spawn کردن child process) تو این محیط (ساندباکس ویندوز) روی timeout گیر می‌کنه** (`[vitest-pool-runner]: Timeout waiting for worker to respond`, بدون خطای دیگه‌ای) — دقیقاً هم‌خانواده‌ی محدودیت شناخته‌شده‌ی #۸ (Playwright/Chromium هم به همین خاطر تو این محیط مشکل داره). فیکس تو `frontend/vitest.config.ts`: `test.pool: "threads"` (بر پایه‌ی `worker_threads`، نه پردازه‌ی جدا) — قابل‌اعتمادتر و سریع‌تر، بدون هیچ ریسکی برای این پروژه چون تست‌هامون نیازی به ایزوله‌سازی سطح پردازه ندارن. حتی با این فیکس هم گاهی زیر بار سنگین (مثلاً وقتی dev server هم‌زمان بالاست) یک اجرای تکی می‌تونه timeout بده و بلافاصله با retry موفق بشه — این flaky-ه، نه رگرسیون واقعی.
26. **`npm ci` می‌تونه در CI fail کنه درحالی‌که همون `package-lock.json` محلی مشکلی نداره** — اگه نسخه‌ی npm محیط dev (که لاک‌فایل رو تولید/آپدیت کرده) با نسخه‌ی npm همراه Node در CI فرق کنه، فرمت لاک‌فایل ممکنه با `npm ci` نسخه‌ی قدیمی‌تر ناسازگار باشه (`Missing: <pkg> from lock file`). قبل از هر افزودن وابستگی جدید فرانت، اگه `EBADENGINE`/`engines` warning دیدی (وابستگی صریحاً یه نسخه‌ی جدید Node می‌خواد)، همون‌جا چک کن که `node-version` در `.github/workflows/ci.yml` باهاش سازگاره — همون فلسفه‌ی نکته‌ی #۲۳ (همیشه با محیط dev واقعی جلو برو، نه یه نسخه‌ی «پایدار» عقب‌تر که فقط تو ذهنته).
