...
### بعد از فاز — سیستم مدرس + enrich دنبال‌کردن‌ها + آپلود آواتار + وبینارهای محبوب

**زمینه:** کاربر قبل از شروع واقعی فاز ۴ (که هدفش جستجوی ChromaDB + صفحه‌ی اصلیه)، حین تست زنده‌ی سایت چند درخواست پشت‌سرهم داد: «چجوری اونهایی که دنبال کردم رو یکجا ببینم؟»، «مدرس‌های محبوب رو تو صفحه‌ی اصلی نشون بده، با کلیک روی عکسشون لیست کامل رویدادهاشون دیده بشه»، «تو پروفایل بتونم عکس خودم رو هم آپلود کنم»، و بعد «وبینارهای محبوب رو هم علاوه بر مدرس‌های محبوب نشون بده». همه‌ی این‌ها زیرساخت لازم برای بخش‌های الگوریتمی صفحه‌ی اصلی (بخش ۱۰ architecture.md) هستن، پس به‌جای رد کردن به فاز ۴، همین‌جا (به‌عنوان یه addendum دیگه‌ی فاز ۳) پیاده شدن — فاز ۴ واقعی (embedding/ChromaDB) هنوز شروع نشده.

**نکته‌ی کشف‌شده:** سیستم «مدرس» (`Instructor` model، جدول `event_instructors`) از فاز ۰ تو دیتابیس تعریف شده بود ولی **هیچ endpoint یا UI ای نداشت** — حتی راهی برای ربط‌دادن مدرس به رویداد وجود نداشت. کل این زیرسیستم از پایه ساخته شد.

**چیزی که ساخته شد:**
- **مدرس (get-or-create با اسم):** `EventCreateIn/EventUpdateIn.instructor_names` دقیقاً مثل `tag_names` — برگزارکننده فقط اسم مدرس رو تایپ می‌کنه (`event_service._get_or_create_instructors`)؛ اگه از قبل با همون اسم دقیق وجود داشته باشه لینک می‌شه، وگرنه رکورد جدید. فیلد جدید در فرم ایجاد رویداد.
- **`GET /instructors`** (مرتب بر اساس `follower_count` زنده — رول‌آپ شبانه‌ی `popularity_score` هنوز نیست، over-engineering برای تعداد کم مدرس فعلی) و **`GET /instructors/{id}`** (بیوگرافی، آواتار، `follower_count`، `is_following` با `get_current_user_optional` جدید در `deps.py`، و لیست رویدادهای منتشرشده‌ی همون مدرس). صفحه‌ی فرانت `app/instructors/[id]/page.tsx` (SSR، مثل صفحه‌ی رویداد) + `FollowInstructorButton.tsx` (کپی الگوی `FollowOrganizerButton.tsx`).
- **`GET /me/follows/details`:** نسخه‌ی enrich‌شده‌ی `/me/follows` قبلی (که فقط `organizer_ids`/`instructor_ids` خام می‌داد — برای دکمه‌ی toggle کافی بود ولی برای نمایش لیست هیچی نداشت). حالا اسم برگزارکننده (`full_name or phone or email`) و اسم/آواتار مدرس هم برمی‌گردونه. صفحه‌ی جدید `app/follows/page.tsx`.
- **آواتار کاربر:** `avatar_url` روی `User` (migration `77be36dfd469`)، `POST /auth/me/avatar` — دقیقاً از همون `validate_and_reencode_image` امن بنر رویداد استفاده می‌کنه (magic-byte، re-encode اجباری، بدون تغییر تابع)، فقط namespace جدا در MinIO (`avatars/{user_id}/...`، تابع جدید `upload_avatar_image`). صفحه‌ی جدید `app/profile/page.tsx` (فرم نام + آپلود آواتار، پیام موفقیت سبز بعد از ذخیره‌ی نام طبق درخواست صریح کاربر). `auth-store.ts` یه فیلد `user: UserOut | null` هم گرفت (علاوه بر `accessToken`) تا `SiteHeader` بتونه آواتار رو کنار لینک «پروفایل» نشون بده و بلافاصله بعد از آپلود/تغییر نام به‌روز بشه (نه فقط بعد از رفرش صفحه).
- **وبینارهای محبوب:** `GET /events?sort=popular` بر اساس `view_count DESC` — **نه `rating_avg`**، چون سیستم امتیازدهی (فاز ۷) هنوز پیاده نشده و همه‌ی `rating_avg`ها صفرن؛ `view_count` تنها معیار محبوبیت واقعی موجوده فعلاً. صفحه‌ی اصلی حالا هم «وبینارهای محبوب» هم «مدرس‌های محبوب» رو نشون می‌ده.
- **فوتر سایت:** `SiteFooter.tsx` (کپی‌رایت + سال شمسی با `Intl` + لوگوی کوچک)، در `layout.tsx` بعد از `{children}`.
- ۱۰ تست integration جدید (۱۹۸ در مجموع): آپلود آواتار (موفق/رد فایل غیرتصویر/نیاز به auth)، get-or-create مدرس (ایجاد + استفاده‌ی مجدد از همون رکورد)، لیست/جزئیات مدرس (شامل `is_following`)، `/me/follows/details`، `sort=popular`.

**دام واقعی کشف‌شده (تکرار همون دام Docker/dev-server قبلی، این‌بار برای uvicorn هم):** بک‌اند dev بدون `--reload` بالا بود؛ بعد از اضافه‌کردن router جدید (`instructors.py`) و import های جدید در `main.py`/`auth.py`، endpointهای جدید تا manual restart اصلاً تو `/openapi.json` ظاهر نمی‌شدن — دقیقاً همون قاعده‌ی نکته‌ی عملیاتی #۲ (CLAUDE.md)، ولی این‌بار برای uvicorn نه فقط next dev.
