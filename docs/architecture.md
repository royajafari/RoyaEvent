...
- **فاز ۳ — بلیط/سفارش/تخفیف/علاقه‌مندی/دنبال‌کردن.** ✅ — مدل‌های `TicketType`/`DiscountCode`/`PlatformDiscountCode`/`Order`/`OrderItem`/`Payment`/`Registration`/`Favorite`/`OrganizerFollow`/`InstructorFollow` + Alembic؛ `ticket_service` (early-bird پویا)، `discount_service` (سطح رویداد+سایت)، `order_service` (create/complete/cancel، پرداخت شبیه‌سازی‌شده)، `social_service` (favorite/follow toggle + لیست)؛ routerهای tickets/orders/social/organizer (attendees list/remove/export CSV) کامل؛ ۱۷۳ تست (unit+integration) پاس. فرانت: `TicketCheckout`/`StickyTicketFooter`/`FavoriteButton`/`FollowOrganizerButton`، صفحه‌ی «بلیط‌های من» (`/tickets`)، داشبورد شرکت‌کنندگان برگزارکننده با export CSV (`/organizer/events/[id]/attendees`)، همه به صفحه‌ی جزئیات رویداد وصل شدن؛ تأیید بصری end-to-end واقعی با دو کاربر (برگزارکننده+خریدار) روی سرور واقعی. کد رویداد به فرمت `RE-XXXXXX` تغییر کرد (پیشوند برند، به درخواست کاربر).
